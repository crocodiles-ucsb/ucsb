...
from src.DAL.auth import check_authorization, generate_tokens, refresh_tokens
...
def _mock_utc_now(mocker):
    ...
    mocker.patch('src.DAL.auth.datetime').utcnow.side_effect = [
        first,
        first,
        second,
        second,
...
async def test_get_tokens_with_valid_username_and_password(tokens):
...
async def test_get_tokens_generates_new_token_pair_for_new_authorization_and_old_access_token_will_not_valid(
    ...
    await generate_tokens(username, password)
    with pytest.raises(AccessTokenOutdatedError):
